...
export function registerClientSQLDebugDatabase(database: ClientSQLDebugDatabase): void {
  // The shared provider registration is inert unless runtime.isDebugEnabled. ClientSQL does
  // not add another authentication layer; access is governed by Valdi's build- and host-gated
  // debugger transport, so embedders must keep that transport disabled outside trusted builds.
