...
Result<SQLiteStatement> prepareStatement(sqlite3* database, const StringBox& sql) {
    ...
    const auto status = sqlite3_prepare_v2(
        database, sql.getCStr(), static_cast<int>(sql.length()), &statement, nullptr);
    if (status != SQLITE_OK) {
...
public:
    explicit ClientSQLDatabaseCoordinator(const Ref<IDispatchQueue>& fallbackQueue);
...
public:
    ...
    ~ClientSQLConnection() override = default;
...
ClientSQLDatabaseCoordinator::ClientSQLDatabaseCoordinator(const Ref<IDispatchQueue>& fallbackQueue) {
    gClientSQLLiveCoordinatorCount.fetch_add(1);
    if (fallbackQueue != nullptr) {
        _writerConnection = makeShared<ClientSQLQueueConnection>(fallbackQueue);
    }
...
void ClientSQLDatabaseCoordinator::transaction(
    ...
    enqueueWriterWork([self = strongSmallRef(this), connection, body = std::move(body), callback = std::move(callback)]() {
        ...
        snap::valdi_modules::client_sql::ClientSQLNativeConnectionProxy::TransactionBodyCallbackFn doneCallback =
            [self, transactionId, callback, completed](Value value, std::optional<StringBox> error) {
...
public:
    Ref<ClientSQLDatabaseCoordinator> coordinatorForDatabasePath(const std::string& databasePath,
                                                                 const Ref<IDispatchQueue>& fallbackQueue) {
        ...
        auto coordinator = makeShared<ClientSQLDatabaseCoordinator>(fallbackQueue);
...
Ref<snap::valdi_modules::client_sql::ClientSQLNativeConnectionProxy> openClientSQLDatabase(
    ...
    auto coordinator = databaseRegistry.coordinatorForDatabasePath(databasePathValue.databasePath, fallbackQueue);
...
public:
    ClientSQLNativeModule(Ref<IDiskCache> diskCache, Ref<IDispatchQueue> workerQueue)
        : _diskCache(std::move(diskCache)), _workerQueue(std::move(workerQueue)) {}
...
ClientSQLNativeModuleFactory::ClientSQLNativeModuleFactory(const Ref<IDiskCache>& diskCache,
    ...
    : _diskCache(diskCache), _workerQueue(workerQueue) {}

ClientSQLNativeModuleFactory::~ClientSQLNativeModuleFactory() = default;

Ref<snap::valdi_modules::client_sql::ClientSQLNativeModule> ClientSQLNativeModuleFactory::onLoadModule() {
    return makeShared<ClientSQLNativeModule>(_diskCache, _workerQueue);
}
