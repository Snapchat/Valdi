...
describe('DebuggerInputMessageHandler', () => {
  ...
  it('rejects handler promises when an input callback throws', async () => {
    const button = new TestElement(81, 'view', {
      accessibilityId: 'throwing-button',
      onTap: () => {
        throw new Error('tap callback failed');
      },
    });
    const handler = makeHandler(makeNode(undefined, [makeNode(button, [])]));
    ...
    ).toBeRejectedWithError('tap callback failed');
  });
