...
from clientsql.sql import collect_create_statements, collect_tables, parse_columns, parse_query_blocks
...
CLIENTSQL_TOOLCHAIN_EXECUTABLE = environment_tool_path("CLIENTSQL_TEST_TOOLCHAIN_GENERATOR")
...
class ClientSQLGeneratorTest(unittest.TestCase):
    ...
    def test_parses_explicit_null_and_generated_column_types(self) -> None:
        columns = parse_columns(
            "payload BLOB NULL, generated_blob BLOB AS (zeroblob(1))",
            {},
        )
        self.assertEqual(
            [
                ("payload", "BLOB", "ArrayBuffer | null", True),
                ("generated_blob", "BLOB", "ArrayBuffer | null", True),
            ],
            [(column.name, column.sql_type, column.ts_type, column.nullable) for column in columns],
        )

    def test_native_contract_exports_expected_declarations(self) -> None:
        native_contract = (
            REPO_ROOT
            / "src"
            / "valdi_modules"
    ...
    def test_native_sqlite_dependency_shape_is_explicit_for_apple_and_default(self) -> None:
        ...
        valdi_toolchain_build = (REPO_ROOT / "bzl" / "valdi" / "BUILD.bazel").read_text(encoding="utf-8")
        generator_build = (REPO_ROOT / "compiler" / "clientsql" / "BUILD.bazel").read_text(encoding="utf-8")
        ...
        self.assertIn('name = "clientsql_runtime_sqlite"', valdi_toolchain_build)
        self.assertIn('name = "clientsql_validation_sqlite"', valdi_toolchain_build)
        self.assertIn('"//bzl/conditions:ios": ["-lsqlite3"]', valdi_toolchain_build)
        self.assertIn('"//bzl/conditions:macos": ["-lsqlite3"]', valdi_toolchain_build)
        self.assertIn('"//conditions:default": ["VALDI_CLIENTSQL_USE_BUNDLED_SQLITE"]', client_sql_build)
        self.assertIn('"//bzl/valdi:clientsql_runtime_sqlite"', client_sql_build)
        self.assertIn('build_setting_default = "@valdi_clientsql_sqlite_316//:sqlite"', valdi_toolchain_build)
        self.assertIn('name = "empty_clientsql_native_implementation"', valdi_toolchain_build)
        self.assertIn('deps = ["//bzl/valdi:clientsql_validation_sqlite"]', generator_build)
        ...
        for declaration in (sqlite_module, sqlite_workspace):
            ...
            self.assertIn('name = "valdi_clientsql_sqlite"', declaration)
            self.assertIn('name = "valdi_clientsql_sqlite_316"', declaration)
    ...
    def test_real_runtime_integration_target_links_generated_smoke_and_native_factory(self) -> None:
        ...
        integration_target = valdi_build[valdi_build.index('name = "test_integration"'):]
        focused_target = valdi_build[valdi_build.index('name = "test_client_sql_runtime_integration"'):]
        aggregate_target = valdi_build[valdi_build.index('name = "test",\n    linkstatic = 1'):]
        self.assertIn("client_sql_smoke:client_sql_smoke_native_desktop", integration_target)
        self.assertNotIn('exclude = ["test/integration/ClientSQLRuntime_tests.cpp"]', integration_target)
        self.assertIn('"test/integration/ClientSQLRuntime_tests.cpp"', focused_target)
        self.assertIn('"//bzl/valdi:clientsql_native_test_library"', aggregate_target)
    ...
    def test_source_generator_preserves_the_existing_toolchain_protocol(self) -> None:
        ...
        toolchain_wrapper = (
            REPO_ROOT / "compiler" / "clientsql" / "src" / "clientsql_toolchain.py"
        ).read_text(encoding="utf-8")
        validator_source = (
        ...
        self.assertIn('sqldelight_compiler = "@valdi_toolchain//:sqldelight_compiler"', toolchain)
        self.assertNotIn("clientsql_sqlite_validator", toolchain_contract)
        self.assertNotRegex(
            toolchain_contract,
            r'"sqldelight_compiler": attr\.label\(\s+executable = True',
        )
        self.assertIn('actual = "@valdi//compiler/clientsql:clientsql_toolchain"', distributed_alias)
        self.assertNotRegex(distributed_alias, r'name = "sqldelight_compiler",\s+srcs = \[\]')
        self.assertIn('metadata: ["version": output]', processor)
        self.assertNotIn("moduleName: bundleInfo.name", processor)
        self.assertNotIn("--sqlite-validator", processor)
        self.assertNotIn("--sqlite-validator", processor_test)
        self.assertNotIn("direct-client-sql-validator-path", compiler_invocation)
        self.assertIn('[generator, "--sqlite-validator", validator, *generator_arguments]', toolchain_wrapper)
        self.assertIn("include_client_sql_tools = False", compiler_invocation)
        ...
        self.assertIn('"-m", "SharedDb"', processor_test)
        self.assertIn('package: "SharedDb"', processor_test)

    def test_toolchain_wrapper_keeps_validator_out_of_the_legacy_compiler_cli(self) -> None:
        self.assertIsNotNone(CLIENTSQL_TOOLCHAIN_EXECUTABLE)
        assert CLIENTSQL_TOOLCHAIN_EXECUTABLE is not None
        self.assertTrue(CLIENTSQL_TOOLCHAIN_EXECUTABLE.is_file())
        self.assertTrue(os.access(CLIENTSQL_TOOLCHAIN_EXECUTABLE, os.X_OK))

        wrapped = subprocess.run(
            [str(CLIENTSQL_TOOLCHAIN_EXECUTABLE), "-version"],
            check=False,
        ...
        self.assertEqual(wrapped.returncode, 0, msg=wrapped.stderr)
        self.assertEqual(direct.returncode, 0, msg=direct.stderr)
        self.assertEqual(wrapped.stdout, direct.stdout)

    def test_toolchain_wrapper_maps_legacy_module_to_the_valdi_bundle(self) -> None:
        self.assertIsNotNone(CLIENTSQL_TOOLCHAIN_EXECUTABLE)
        assert CLIENTSQL_TOOLCHAIN_EXECUTABLE is not None
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            database = (output / "SharedDb.ts").read_text(encoding="utf-8")
            expected_namespace = hashlib.sha256(b"MyBundle").hexdigest()[:24]
            legacy_namespace = hashlib.sha256(b"SharedDb").hexdigest()[:24]
            self.assertIn(f'const DATABASE_NAMESPACE = "{expected_namespace}";', database)
            self.assertNotIn(f'const DATABASE_NAMESPACE = "{legacy_namespace}";', database)

    def test_projectsync_owns_generated_sql_sources_without_compile_output_conflicts(self) -> None:
        generated_sql_dir = (
            REPO_ROOT
            / "valdi"
            / "testdata"
        ...
        for filename in ("UserTypes.ts", "UserQueries.ts", "TestDb.ts"):
            self.assertTrue(
                (generated_sql_dir / filename).is_file(),
                msg=f"Missing project-sync ClientSQL output: {filename}",
            )
    ...
    def test_preserves_schema_and_types_for_parameterized_joined_tables(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            sql_dir = root / "sql"
            db_dir = sql_dir / "TestDb"
            ...
            (db_dir / "Feed.sq").write_text(
                ...
                CREATE TABLE post (
                  ...
                  ratio DECIMAL(18, 4),
                  CONSTRAINT positive_amount CHECK ((amount >= 0) AND (ratio IS NULL OR ratio >= 0)),
                  UNIQUE (title, amount)
                ) WITHOUT ROWID;
                ...
                SELECT *
                FROM post
                JOIN author ON author.author_id = post.post_id;

                selectExplicit:
                SELECT p.title, a.name AS author_name, p.amount
                FROM post AS p
                JOIN author AS a ON a.author_id = p.post_id;

                selectLeftJoined:
                SELECT *
                FROM post
                LEFT JOIN author ON author.author_id = post.post_id;

                selectReserved:
                SELECT post_id FROM post
                WHERE post_id = :await AND title = :async AND amount = :as AND ratio = :of;

                selectMaskedSources:
                SELECT * FROM post -- JOIN author
                WHERE 'JOIN author' = 'JOIN author' /* JOIN author */;

                selectNestedSource:
                SELECT * FROM post
                WHERE EXISTS (
                  SELECT 1 FROM author WHERE author.author_id = post.post_id
                );

                selectCompound:
                SELECT * FROM post
                UNION ALL
                SELECT author_id, name, "primary", NULL FROM author;
                """,
                encoding="utf-8",
            )
            ...
            types = (out_dir / "FeedTypes.ts").read_text(encoding="utf-8")
            self.assertIn("title: string;", types)
            self.assertIn("amount: number;", types)
            self.assertIn("ratio: number | null;", types)
            joined_start = types.index("export interface SelectJoinedRow")
            joined_row = types[joined_start:types.index("\n}", joined_start)]
            explicit_start = types.index("export interface SelectExplicitRow")
            explicit_row = types[explicit_start:types.index("\n}", explicit_start)]
            left_joined_start = types.index("export interface SelectLeftJoinedRow")
            left_joined_row = types[left_joined_start:types.index("\n}", left_joined_start)]
            ...
            self.assertIn("selectJoined(): Promise<SelectJoinedRow[]>", queries)
            self.assertIn("selectMaskedSources(): Promise<Post[]>", queries)
            self.assertIn("selectNestedSource(): Promise<Post[]>", queries)
            self.assertIn("selectCompound(): Promise<Post[]>", queries)
            self.assertIn(
                "selectReserved(_await: number, _async: string, _as: number, _of: number | null)",
                queries,
            )
            self.assertNotIn("export interface SelectMaskedSourcesRow", types)
            self.assertNotIn("export interface SelectNestedSourceRow", types)
            self.assertNotIn("export interface SelectCompoundRow", types)
            database = (out_dir / "TestDb.ts").read_text(encoding="utf-8")
            self.assertEqual(2, database.count("WITHOUT ROWID;"))
            self.assertIn("CHECK ((amount >= 0) AND (ratio IS NULL OR ratio >= 0))", database)

            entrypoint = root / "reserved-identifiers.ts"
            entrypoint.write_text("console.log('ok');\n", encoding="utf-8")
            compile_result = self.run_generated_typescript(root, entrypoint)
            self.assertEqual(
    ...
    def test_schema_lexer_preserves_quoted_comment_markers_and_ignores_commented_labels(self) -> None:
        sql = """
        /*
        fakeQuery:
        SELECT 1;
        */
        CREATE TABLE "ledger--item" (
          id INTEGER NOT NULL PRIMARY KEY,
          "note--text" TEXT NOT NULL DEFAULT 'a--b',
          details TEXT DEFAULT '/* literal */',
          score INTEGER -- A semicolon here is a comment, not a statement boundary: ;
        );

        realQuery:
        SELECT "note--text", details FROM "ledger--item";
        """

        tables = collect_tables([sql], {})
        self.assertEqual(["ledger--item"], [table.name for table in tables.values()])
        self.assertEqual(
            ["id", "note--text", "details", "score"],
            [column.name for column in tables["ledger--item"].columns],
        )

        create_statements = collect_create_statements([sql])
        self.assertEqual(1, len(create_statements))
        self.assertIn("DEFAULT 'a--b'", create_statements[0])
        self.assertIn("DEFAULT '/* literal */'", create_statements[0])
        self.assertTrue(create_statements[0].rstrip().endswith(");"))

        query_blocks = parse_query_blocks(sql)
        self.assertEqual(["realQuery"], [name for name, _ in query_blocks])
        self.assertIn('FROM "ledger--item"', query_blocks[0][1])

    def test_query_inference_handles_joins_subqueries_and_quoted_identifiers(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            sql_dir = root / "sql"
            db_dir = sql_dir / "TestDb"
            db_dir.mkdir(parents=True)
            (db_dir / "Inference.sq").write_text(
                """
                CREATE TABLE numeric_item (
                  id INTEGER NOT NULL PRIMARY KEY,
                ...
                CREATE TABLE "ledger--item" (
                  id INTEGER NOT NULL PRIMARY KEY,
                  "JOIN text_item" TEXT NOT NULL
                );

                selectJoinedValue:
                SELECT n.id AS "number id", t.value AS [text value]
                FROM numeric_item AS n
                JOIN text_item AS t ON t.id = n.id
                WHERE t.value = :value;

                selectScalarProjection:
                SELECT (SELECT max(t.id) FROM text_item AS t) AS "max id",
                       n.id AS `number id`
                FROM numeric_item AS n;

                selectQuotedSource:
                SELECT "ledger--item"."JOIN text_item" AS "note--text"
                FROM "ledger--item"
                WHERE "ledger--item"."JOIN text_item" = :note;
                """,
                encoding="utf-8",
            )
            ...
            types = (out_dir / "InferenceTypes.ts").read_text(encoding="utf-8")
            self.assertIn('"number id": number;', types)
            self.assertIn('"text value": string;', types)
            self.assertIn('"max id": number | null;', types)
            self.assertIn('"note--text": string;', types)

            queries = (out_dir / "InferenceQueries.ts").read_text(encoding="utf-8")
            self.assertIn("selectJoinedValue(value: string)", queries)
            self.assertIn("selectQuotedSource(note: string)", queries)
            self.assertIn(
                'watchQuery(["numeric_item", "text_item"], () => this.selectJoinedValue(value), listener)',
                queries,
            )
            self.assertIn(
                'watchQuery(["ledger--item"], () => this.selectQuotedSource(note), listener)',
                queries,
            )
